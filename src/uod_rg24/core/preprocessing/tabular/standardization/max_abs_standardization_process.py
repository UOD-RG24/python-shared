...
class MaxAbsScalerPartialFitProtocol(Protocol):
    def partial_fit(
        self,
        X: pd.DataFrame,
    ) -> object: ...
...
def max_abs_standardization_process(
    ...
    partial_fit_scaler = cast(
        MaxAbsScalerPartialFitProtocol,
        scaler,
    )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for chunk in pd.read_csv(
            ...
            partial_fit_scaler.partial_fit(
        ...
        steps.append(
            ProcessStepModel(
                step="max_abs_scaler_fit",
                startedAt=step_started_at,
                completedAt=utc_now(),
                durationMs=(perf_counter() - step_timer) * 1000,
                message=(
                    f"Fitted MaxAbsScaler using "
        ...
        logger.info(
            "Uploading MaxAbs scaled dataset. destination_blob=%s",
            output_blob_path,
        )
