...
from uod_rg24.models.preprocessing.tabular.sample_selection.retain_complete_samples_process_models import (
    RetainCompleteSamplesProcessInfoModel,
...
def retain_complete_samples_process(
    blob_service_client: BlobServiceClient,
    input_blob: InputModel,
    output_blob: OutputModel,
    retain_complete_samples_process_request: RetainCompleteSamplesProcessRequestModel,
) -> RetainCompleteSamplesProcessResponseModel:
    del retain_complete_samples_process_request
    ...
    normalized_input_extension: str = f".{input_blob_extension.lstrip('.').lower()}"

    if normalized_input_extension not in {".csv", ".tsv"}:
        raise ValueError(
            f"Unsupported input dataset type: {normalized_input_extension}. "
            "Only .csv and .tsv files are supported."
        )

    output_blob_extension: str = output_blob.extension or input_blob_extension

    normalized_output_extension: str = f".{output_blob_extension.lstrip('.').lower()}"

    if normalized_output_extension not in {".csv", ".tsv"}:
        raise ValueError(
            f"Unsupported output dataset type: {normalized_output_extension}. "
            "Only .csv and .tsv files are supported."
        )
    ...
    input_separator: str = "\t" if normalized_input_extension == ".tsv" else ","

    output_separator: str = "\t" if normalized_output_extension == ".tsv" else ","
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        input_path: str = os.path.join(
            temporary_directory,
            f"input{normalized_input_extension}",
        )

        output_path: str = os.path.join(
            temporary_directory,
            f"output{normalized_output_extension}",
        )
    ...
    selection_info = RetainCompleteSamplesProcessInfoModel()

    processing_completed_at = utc_now()

    return RetainCompleteSamplesProcessResponseModel(
        success=True,
        ...
        inputFile=FileProcessInfoModel(
            ...
            extension=normalized_input_extension.lstrip("."),
